...
> ```kubectl get ingress```
> ```kubectl describe myapp-ingress # look at backends output```
> - Does the Service connect to Pods addressing the right label
> ```kubectl get svc```
> ```kubectl describe myapp-svc```
> ```curl <ip-from-get svc command> # to test if you can get to the deployment from the service```
> ```kubectl get pod myapp-xxxx --show-labels```
...
## Netowrk Policy
> Use ```kubectl get netpol -n namepsace``` to list Networkpolicy for a specific namespace
